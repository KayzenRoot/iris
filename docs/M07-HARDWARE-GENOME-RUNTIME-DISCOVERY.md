...
The canonical JSON transport is `iris-m07-json-v1`. It uses a closed static registry of tags, sorted object fields and maps, normalized text, exact schema fields and bounded nesting/items/bytes. Duplicate JSON keys, unknown tags, noncanonical bytes, non-finite numbers and arbitrary object values fail closed. The SHA-256 `GenomeId` binds canonical content. Consumer projections bind a named/versioned contract, schema, facts and omission reasons. Each projection contract explicitly selects fact keys, declares required keys and controls capability inclusion; omissions distinguish facts not selected by the contract from facts absent from the source. This is the context-slice boundary, so a consumer receives only the requested material and can account for both omission kinds. Their static fingerprint excludes dynamic telemetry while retaining the source Genome ID as separate lineage metadata.
