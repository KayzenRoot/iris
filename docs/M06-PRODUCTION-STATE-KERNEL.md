...
Reuse admission binds an exact M02 `BuildPlan`, its reuse step and `ReuseReceipt`, the M06 candidate materialization, and verified evidence for every mandatory material fingerprint dimension. The admission is decision-scoped: work selection must present the same `decision_id`, and any positive verification used for reuse must bind the exact candidate materialization. Frontier evolution may stay equally conservative or become more conservative; becoming less conservative requires a separate stronger-evidence admission. Partial repair binds an exact M02 repair node and `DependencySlice`; M06 cannot widen that slice. Mixed reconstruction retains both rebuilt and reused ancestry.

Reproducibility is an explicit class. Exact-byte regeneration requires pinned workflow/model/toolchain references and positive completeness evidence for material inputs and execution dimensions. A seed alone is not proof. Retained restoration requires an exact materialization; exact-byte success binds the output materialization digest to observed digest evidence. Exact semantic-state claims bind a versioned semantic contract and result verification. `EQUIVALENT_WITHIN_CONTRACT` additionally requires a versioned equivalence contract plus observed equivalence-verification evidence; semantic/equivalence failure, unavailable dependencies/toolchain, policy blocking, or unknown divergence cannot be reported as successful equivalence. Historical permission never substitutes for current M53/M54 authorization.

## Lineage, rollback, cleanup, and release

Lineage graphs bind a complete fresh graph to exact closure evidence and reject cycles. Cleanup evaluates the protected closure categories, explicit roots, active retention pins, and exact graph epoch/fingerprint. Unknown, partial, stale, or protected state is not eligible for cleanup. Before physical completion is recorded, deletion authorization must be revalidated against current lineage, protected closures, retention evidence, and policy. A changed graph, protected closure, retention pin, or policy makes the old authorization stale or revoked instead of silently authorizing deletion. M06 records logical retirement and M55 authorization/revalidation/completion evidence; it does not delete bytes.

Rollback targets a distinct exact M02 historical snapshot within the same project/production and produces new history instead of rewriting old masters. A complete rollback must bind the exact output revision proven by an explicit `ROLLBACK` reconstruction manifest that cites the target snapshot. Release capsules bind a coherent M02 transaction/snapshot/build closure: project and production agree, the build graph and target revision match the release snapshot, and every master materialization has both verified integrity and current known-availability evidence retained in the capsule. M06 validates closure; it does not publish, release, or promote.
