...
## Next-module rule
M05 planning starts from the 2677-test exact-main baseline. Final Technology Review consolidates all future proof obligations into `F-M05-01..25`, covering all 150 DNAX design-history candidates exactly once. The Forward Compatibility Scan must prove that these families do not duplicate downstream ownership and that future implementation tests can cover all 150 hard invariants through consolidated proof families rather than one-class-per-DNAX expansion. Concrete M05 implementation tests remain unadmitted until contract freeze.
